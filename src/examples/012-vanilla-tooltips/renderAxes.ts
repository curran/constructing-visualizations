...
export function renderAxes(
  ...
  // The axes group is a React-rendered container for the axes. The
  // axis generators fill the positioned x and y axis groups with a
  // domain line and ticks.
  axes.select<SVGGElement>('g.x-axis').call(axisBottom(xScale));
  axes.select<SVGGElement>('g.y-axis').call(axisLeft(yScale));
}
